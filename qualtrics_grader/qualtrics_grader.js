...
if (parent.allowReset) {
  postSurveyHTML +=
    ' If you would like to change your answers, use the "Reset" button at the bottom to restart the survey from the beginning.';
}
...
$(document).ready(function () {
  ...
  // Making sure the survey source is set. Usually very quick.
  let timer = 0;
  var waitForSource = setInterval(function () {
    if (parent.qualtricsSurveySource) {
      clearInterval(waitForSource);
      ...
      $("#QualtricsSurvey").attr("src", parent.qualtricsSurveySource);
      JSProblemState.survey_url = parent.qualtricsSurveySource;
    }
    // Give it 3 seconds to find the survey location variable; log error if you can't.
    timer += 0.25;
    if (timer > 3) {
      clearInterval(waitForSource);
      console.log(
        "Couldn't find qualtricsSurveySource variable - check your problem setup."
      );
      $("body").html("<p>This problem has a configuration issue.</p>");
    }
  }, 250);
  ...
  function hearQualtricsSurveyEnd(e) {
    ...
    if (typeof e.data == "string") {
      ...
      if (e.data.source === "HX_Qualtrics_Survey") {
        JSProblemState.score = Math.max(
          JSProblemState.score,
          e.data.score / 100
        );
      }
    }
