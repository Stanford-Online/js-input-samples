// Much of this is copied from the Matching problem type,
// so some of the terminology might be weird.
// Global variables elementsLeft and elementsRight are read in through
// another link in the the HTML.
...
function loadElementsSighted() {
  ...
  for (let key in elementsLeft) {
    tableArrayLeft.push(key);
  }

  for (let key in elementsRight) {
  ...
  for (let i = 0; i < tableArrayRight.length; i++) {
    label =
...
function handleDrop(event, ui) {
  ...
  for (var i = 0; i < JSProblemState.pairings.length; i++) {
    if (_.isEqual(JSProblemState.pairings[i], thisPairing)) {
      // Uses underscore.js
      existingMatch = true;
